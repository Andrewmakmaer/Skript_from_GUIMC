...
ConcatenationOfString(list, firstemptystr, firstfiledstr, numlaststr);
return firstfiledstr - firstemptystr; //последняя строка у нас сместилась — возвращаем разницу
}
...
function Main() {
  ...
  for (i = 2; i <= numlaststr; ++i) { // читаем строки начиная со второй и до последней
    if (!list.getRange(i, 4).isBlank() && !list.getRange(i, 3).isBlank()) { // проверяем, что у нас в статусе "+" — соббщение   
